...
_It is not essential to install all of these packages, but installing them ensures the smooth working of ns-3_
...
`tar` _is available by default on Linux and macOS systems. Windows users need to find an alternative._
...
_Installing `cppyy==3.1.2` also runs fine, but I was getting some materialize symbols warnings. Turns out using `cppyy==2.4.2` solves this issue._
...
1.  One is to run a ns-3 shell
    1. `./ns3 shell`
    2. `python3 examples/wireless/mixed-wired-wireless.py`
2.  Other is to use the ‘run’ option to ns-3
    1. `./ns3 run examples/wireless/mixed-wired-wireless.py`
...
_The above docker command will run the container, attach it to your current terminal session and mount the current directory opened in your terminal to the code directory in the root ns-3 build folder_
...
PS: If you get any `Failed to materialize symbols` error, then its totally fine to ignore it for now.
